...
public class ExerciseServiceImpl extends ServiceImpl<ExerciseMapper, Exercise> implements IExerciseService {
    ...
    public Intervals getExerciseIntervalsInOneDay(String category,String userId, String date) {
        ...
        for (ExerciseDTO list : lists) {
            ...
            Map<LocalDateTime, LocalDateTime> tmp=new HashMap<>();
            tmp.put(date1,date2);
            formattedLists.add(tmp);
    ...
    @Override
    @Transactional
    public Integer addExercise(String userId, Double longitude, Double latitude, HttpServletRequest request) {
    ...
    //TODO
    @Override
    public Integer finishExercise(String userId,String token) {//它应该要结束当前用户的所有运动记录
        SimpleRedisLock lock = new SimpleRedisLock("user:"+userId, stringRedisTemplate);
        lock.unlock();
        ...
        String url = "http://localhost:81/api/health/getProfile";
        // 2.2.发送http请求，实现远程调用
        Response<Profile> profile0 = restTemplate.getForObject(url, Response.class,entity);
    ...
    public RealTimeSportDTO getRealTimeSport(String userId,Double longitude,Double latitude) throws Exception {
        ...
        int duration = (int) Duration.between(start_time,now).getSeconds();
